import { CalendarClock, CalendarDays, CircleDollarSign, Earth, Mail, Phone, Signpost, UsersRound } from 'lucide-react';
...
function MenuInformacoes() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (companyID) {
                try {
                    const response = await fetch(`http://localhost:8080/empresa/find/id/${companyID}`);
                    if (!response.ok) {
